...
- `<foo>` represent a required argument, called `foo`, that must be replaced with a valid argument without including `< >`
  - if the argument is `<InstDiff>`, that argument must follow the same naming scheme used for [sections in .chart files](https://github.com/TheNathannator/GuitarGame_ChartFormats/tree/main/doc/FileFormats/.chart)
