...
## Key Paths and how Downbeat evaluates them
...
To explain why this is important it's time to explain what *Key Paths* are.

Key Paths are *Paths that have a time value associated with it*. It is called like this because fundamentally they have the same role as KeyFrames in other programs.
Each pair is written as `<time> = <Path>`:
- `<time>` is a numeric value that represents the starting time for this Key Path
...
In *most* cases (see [below](#how-paths-are-handled-for-notes)) Downbeat will take these Key Paths and use them to get a value (in these examples a point in 3D space) to set as a parameter for something.
...
**`playback time` is used to interpolate between two consecutive Key Paths in a linear way between the specified `<time>` values of the relative Key Paths**.
...
Once calculated `p`, Downbeat will take the Key Path associated with 1 and use `p` to calculate the final value to use.
...
This table makes it possible to see the difference between the two example Key Paths, because if they were inverted (`1 = M 1 2 3 L 0 0 0`, `2 = M 0 0 0 L 1 2 3`) the resulting table would be:
...
TODO: talk about how playback time is normalized


### Edge cases
Let's assume that we have the following list of Key Paths for discussing the edge cases; the Path part of them is not relevant, just the time values associated with them:
...
For this reason the last Key Path is less useful compared to the others, because the path declared for it will basically be unused except for the end.

*Case 3:* What happens when the list only has one Key Path?
...
In the context of Notes and Notegroups, the way paths are interpreted is different from the previous explaination.
This is because Downbeat has two way of using the data contained in a Key Path:
- `single mode`: This is the method explained [above](#key-paths-and-how-downbeat-evaluates-them), when every frame paths are used to calculate a **single value** to set as the property for something during gameplay. 
- `meta mode`: This is a special method used for notes where every frame paths are used to calculate an **entire path** as a base for getting multiple values.

From the paths listed in [NoteGroups.md](NoteGroups.md), `Position`, `Rotation`, `Scale` and all `Easing` paths are interpreted in `meta mode`, while all the rest of the paths are in `single mode`.
To explain this difference in more detail in this chapter we'll explain how position is set for a note, but the same reasoning applies also for rotation and scaling.

Every frame Downbeat:
1. uses `Position` and `PositionBlending` to construct an entire path, called `P`, to determine all possible note positions for this frame.
2. calculates which notes are visible to the player and assigns them a "progress" value named `a`, where `a is 0` when the note should be hit by the player and `a is 1` when it's in the farthest point in the highway.
3. `a` is then used as input to `PositionEasing` in order to get the value `b`, obtaining how far the note is along the path.
4. finally, `b` is used as input to `P` in order to get the final position of the note in question.

In order to construct the path `P` on step 1, Downbeat uses the Key Path defined immediately before and immediately after `playback time`.
Then it uses `playback time` as input to `PositionBlending` (interpreted in `single mode`) to get a value between `0` and `1`.
This blending value is used as the parameter for linearly interpolating the two paths: 
- If this blending value is `0` (the default if not specified), Downbeat will use the Key Path before `playback time`.
- If this blending value is `1` Downbeat will use the Key Path after `playback time`.
- Otherwise a mix of the two paths will be considered.

## ChronoPaths and path names
So, now that we know what Paths are, how Key Paths are defined and all the trouble of how time values are used, there's only one thing left to explain: how does Downbeat in practice use all of this?
...
Let's define this more rigorously: a `ChronoPath` is *an object containing a name, a dimention value and a collection of Key Paths.
For each Key Path, the Path inside must contain commands that specify points with the same dimention as the one contained inside the ChronoPath Object*.
...
Once a ChronoPath is found, the playback time of the current song will be used as `t` value to determine the correct [time range](#key-paths-and-how-downbeat-evaluates-them) to consider, to finally get the position of the cursor.
